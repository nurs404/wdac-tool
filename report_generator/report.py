...
def _compact_pc_list(pcs: list[str]) -> str:
    ...
    for pc in pcs:
        m = re.search(r'(\d+)$', pc)
        if m: nums.append((int(m.group(1)), pc))
        else: non_num.append(pc)
    if not nums: return ", ".join(pcs)
    nums.sort()
    ranges, start, end = [], nums[0][0], nums[0][0]
    prefix = re.sub(r'\d+$', '', nums[0][1])
    for n, _ in nums[1:]:
        if n == end + 1: end = n
        else:
